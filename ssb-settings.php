 ...
 */
	...
	if(empty($_POST['ssb_hidden']))
	...
	else
		{
			...
			if(wp_verify_nonce( $nonce, 'social_share_button_nonce' ) && $_POST['ssb_hidden'] == 'Y') {

			    //Form data sent
			$ssb_share_content_display = sanitize_text_field($_POST['ssb_share_content_display']);
			update_option('ssb_share_content_display', $ssb_share_content_display);			

			$ssb_share_content_themes = sanitize_text_field($_POST['ssb_share_content_themes']);
			update_option('ssb_share_content_themes', $ssb_share_content_themes);
			
			$ssb_share_content_position = sanitize_text_field($_POST['ssb_share_content_position']);
			update_option('ssb_share_content_position', $ssb_share_content_position);
			
			$ssb_share_total_count_display = sanitize_text_field($_POST['ssb_share_total_count_display']);
			update_option('ssb_share_total_count_display', $ssb_share_total_count_display);			
			
			$ssb_share_content_icon_margin = sanitize_text_field($_POST['ssb_share_content_icon_margin']);
			update_option('ssb_share_content_icon_margin', $ssb_share_content_icon_margin);
			

			if(!empty($_POST['ssb_share_filter_posttype']))
				{
					$ssb_share_filter_posttype = stripslashes_deep($_POST['ssb_share_filter_posttype']);

					foreach ($ssb_share_filter_posttype as $posttype=>$posttype_value){

						$ssb_share_filter_posttype_new[$posttype] = sanitize_text_field($posttype_value);
                    }

					$ssb_share_filter_posttype = $ssb_share_filter_posttype_new;
			...
			$ssb_share_target_tab = sanitize_text_field($_POST['ssb_share_target_tab']);
			update_option('ssb_share_target_tab', $ssb_share_target_tab);	
			
			$ssb_social_sites = stripslashes_deep($_POST['ssb_social_sites']);

            foreach ($ssb_social_sites as $site_key=>$site_value){

                $ssb_social_sites_new[$site_key] = sanitize_text_field($site_value);
            }

				$ssb_social_sites = $ssb_social_sites_new;
			...
			$ssb_social_sites_domain = stripslashes_deep($_POST['ssb_social_sites_domain']);
				...
				foreach ($ssb_social_sites_domain as $domain_key=>$domain_value){

					$ssb_social_sites_domain_new[$domain_key] = sanitize_text_field($domain_value);
				}


				$ssb_social_sites_domain = $ssb_social_sites_domain_new;
			...
			$ssb_share_archive_display = sanitize_text_field($_POST['ssb_share_archive_display']);
			update_option('ssb_share_archive_display', $ssb_share_archive_display);
			
			$ssb_share_home_display = sanitize_text_field($_POST['ssb_share_home_display']);
			update_option('ssb_share_home_display', $ssb_share_home_display);
			
			$ssb_share_excerpt_display = sanitize_text_field($_POST['ssb_share_excerpt_display']);
